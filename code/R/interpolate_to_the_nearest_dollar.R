...
interpolate_to_the_nearest_dollar <- function(data, x_col, y_col, x_out) {
    ...
    keys <- data %>% dplyr::select_(group_vars(data)) %>% unique() %>% as.data.frame()
    K <- nrow(keys)
    #'
    ##########################
    ...
    for (k in 1:K) {
        ...
        x <- data_subset %>% dplyr::select_(x_col, y_col) %>% drop_na() %>% .[[x_col]]
        y <- data_subset %>% dplyr::select_(x_col, y_col) %>% drop_na() %>% .[[y_col]]

        #' Apply interpolation
        y_out <- interpolation.kernel.customized(x, y, x_out)[["yout"]]

        #' Store results
        interpolated_result <- cbind(x_col = x_out, y_col = y_out)
        colnames(interpolated_result) <- c(x_col, y_col)
        interpolated_result <- cbind(keys[rep(k, nrow(interpolated_result)), ], 
                                     interpolated_result)
