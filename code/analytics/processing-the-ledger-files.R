...
for (l in 1:length(file_paths)) {
    ...
    ledger_data <- switch(ledger %>% .$payment_selection_criteria %>% unique(),
        max_total_ratio = {
            ledger_feature_extractor(ledger)
        },
...
results <- results %>% dplyr::group_by(database_name, model_inducer, cost_function_type, payment_selection_criteria, repetition)
interpolated_results <- interpolate_to_the_nearest_dollar(
    data = results,
    x_col = "cost_so_far",
