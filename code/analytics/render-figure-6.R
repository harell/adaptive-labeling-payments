...
source(file.path(getwd(), "code", "analytics", "processing-the-metadata-files.R"))
...
##########################
x_axis <- "cost_so_far"
y_axis <- c("AUC_holdout", "train_set_label_quality", "observations_so_far")[1]
#'
################
# Get the data #
################
...
expected_combinations <-
    ...
    dplyr::arrange(database_name, cost_function_type)
dataset_combinations <-
    dataset %>%
    dplyr::select(database_name, cost_function_type) %>%
    unique() %>% 
    dplyr::arrange(database_name, cost_function_type)
assertive::assert_are_set_equal(expected_combinations, dataset_combinations)
#' Test that the selected columns have no NA
assertive::assert_any_are_not_na(dataset %>% dplyr::select_(x_axis, y_axis))
#'
#################
# Preprocessing #
#################
...
plot_data <-
    dataset %>%
    dplyr::group_by(database_name, cost_function_type, payment_selection_criteria, x_axis) %>%
    dplyr::summarise(y_axis = mean(y_axis))
fig6 <- plot_data %>%
