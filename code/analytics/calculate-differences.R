...
reference_strategy <- c("random", "max_total_ratio")[1]
#'
################
# Get the data #
################
...
results_wide <- 
    params[, -8] %>%
    tidyr::spread(key = "payment_selection_criteria", value = "Cost") %>%
    dplyr::arrange(DATABASE_NAME, model_inducer, cost_function_type)
## Save on hard disk
