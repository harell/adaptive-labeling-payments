...
repetition_stage_4 <- function() {
    ...
    while ((cost_so_far <= max_total_cost) &
        ...
        new_record <- bind_cols(new_record, data.frame(payment_selected = pay_per_label)) %>% as.data.frame()
        #'
        #############################################
        ...
        new_record <- bind_cols(new_record, data.frame(AUC_holdout_set = calculated_AUC)) %>% as.data.frame()
        cat("\n", "AUC =", calculated_AUC)
        cat("\n---")
        # Fix SVM bug
